...
启用飞书远程助手后，飞书会将发送给该机器人的文字消息通过加密长连接传到这台 Mac；浮屿的文字回复会发送回同一飞书会话。App Secret 保存于 macOS 钥匙串。飞书功能默认关闭，浮屿不会把本机文件或录音自动发送到飞书。
...
When the Feishu remote assistant is enabled, text sent to the configured bot is delivered to this Mac over Feishu's encrypted long connection, and FuYu sends text replies back to the same chat. The App Secret is stored in macOS Keychain. The feature is off by default and never uploads local files or recordings automatically.
