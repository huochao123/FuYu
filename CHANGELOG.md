...
- 新增 Voicebox 本地语音通道：Apple Silicon 可使用 Whisper Small 本地识别和 Qwen TTS 0.6B 本地声音克隆，日常语音不必上传到 ASR/TTS 云端。
- 将浮屿原 MiMo 默认“冰糖”音色导出为 15 秒、24 kHz 本机参考样本，并创建“浮屿 · 冰糖”Voicebox 克隆档案；MiMo“冰糖”与系统语音继续作为两级回退。
- Voicebox 未运行时，浮屿会在明确语音使用时后台启动应用，不会因文字聊天触发麦克风、语音授权或 Voicebox。
- 针对 Voicebox 0.5.0 的 MLX 线程归属故障增加单模型卸载重试；本地引擎失败时识别回退 MiMo，发声依次回退 MiMo 与系统语音，不会结束连续对话。
- 修复 Voicebox 模式在没有检测到用户声音时也可能提交空录音的条件优先级错误。
- 增加 Voicebox 服务、克隆发声、真实长录音和生成音频回听识别测试；本机实测 23.7 秒长录音热识别约 1.9 秒。

## v0.9.3 — 2026-07-19

- 修复长语音被固定 6 秒 MiMo 校正超时误判为“没有听清”的问题；真实日志确认失败轮次均没有 Apple 临时文字，不能使用短超时回退。
- MiMo ASR 超时改为根据真实录音时长动态计算：最少 18 秒、录音越长等待越充足、最长 90 秒；成功时仍立即返回，不会固定等待。
- 没有备用文字时，首次 MiMo ASR 请求失败会自动重试一次；已有 Apple 临时文字时继续快速回退，兼顾可靠性和速度。
- 增加浮屿、赫尔墨斯、MiMo、Codex 等专有名词的保守本地纠正，不修改“赋予权限”等正常中文词义。
- 语音连接测试同时覆盖短句和长句，防止再次出现“几个字可以、稍长命令失败”的回归。
- 收尾固定等待从 320 毫秒降到 120 毫秒，并取消提交前额外 650 毫秒停顿；每个正常语音轮次减少约 850 毫秒本机固定等待。
- 语音播报与屏幕内容分层：检测结果播报结论、影响和下一步，执行结果播报成败与关键结果；参数、路径、ID 和长数字继续只显示不朗读。
- 修复真实麦克风以 96 kHz 录制时，约 10 秒以上音频因请求体过大被 MiMo 返回 400 的问题；长录音现在按完整音频帧切成安全小段识别后合并，确定性的 4xx 错误不再重复上传等待。

## v0.9.2 — 2026-07-19

- 修复主界面与连续对话识别完成后偶发未送交 AI 的提交链路；自动触发器在进入最终化前释放自身引用，并增加逐轮提交日志。
- 连续语音改为明确会话契约：静默、空识别、识别中断、麦克风重建、语音生成失败和普通错误都不会自行结束；只有手动停止或明确结束口令才关闭。
- 增加“语音取消、取消语音对话、取消通话”等结束口令；“取消执行”只取消待授权操作，不再误伤整个语音会话。
- MiMo 识别校正增加 6 秒独立超时；校正是增强项而非提交前置条件，超时或失败会立即采用现场识别文字继续送交 AI。
- 通知自动消失、关闭历史记录和临时错误卡不再通过通用复位清除语音会话；关闭临时卡后会恢复收音。
- 连续语音测试扩展为四轮真实麦克风重建，并新增四轮最终文字送交 AI 的确定性回归测试。

## v0.9.1 — 2026-07-19

- 重做聊天意图防线：粘贴更新说明、功能清单和聊天记录只会进入讨论，不会再因出现“应用健康”等词误启动本机检测。
- “你人还在吗”等在线确认不再覆盖当前任务；“处理、按建议做、你自己处理”会承接最近一次真实检测建议，而不是重复扫描。
- 能力回答改为读取当前注册表，准确展示十三项电脑管家工具，不再沿用旧版“九项能力”的硬编码说法。
- 同一检测在短时间内不会被模型重复启动；助手会先展示已有结论、证据、影响与下一步，需要刷新时再明确重查。
- 电脑管家结果补齐建议、收益、风险和可执行入口，聊天侧可直接说“按建议处理”或“暂不处理”完成闭环。
- 修正绾宁人格中的贬低、质疑和推责表达；保留嘴硬心软的风格，但错误和排查场景必须尊重用户、承担问题并给出下一步。
- 长扫描结果进入模型前会自动压缩，完整明细保留在状态屏；旧的超长执行记忆会自动迁移，减少连续聊天变慢和超时。
- 修复进程数量较多时系统进程列表可能填满通信管道、导致系统体检或电池检测一直等待的问题。
- “暂时不处理、先不处理”等否定表达会明确保留结果且不执行；打开活动监视器不再被标记成异常已经解决。
- “为什么、如果、会怎么做、先别真的调”等解释和假设表达统一只讨论，不会因为包含“处理、音量”等操作词而误执行。
- 电池检测改为展示系统健康状态、最大容量、循环次数和当前电量，并严格区分睡眠阻止项与高负载进程，避免把电量误当健康度。
- 新增真实问题语料回归：功能说明误触发、在线确认丢焦点、处理指令重复检测、能力数量过期及不当人格话术均纳入自检。

## v0.9.0 — 2026-07-19

- 主窗口重构为窄导航、中央状态工作区和右侧智能中枢三栏结构；声音动画不再占据近半屏，状态、功能和后台任务拥有更合理的空间。
- 新增总览首页：实时 Mac 状态屏、浮屿判断、安全守护、快捷操作和最近健康事件集中展示。
- 电脑管家扩展为十三项真实本机工具，新增应用健康、电池与发热、隐私权限、故障现场；每项结果都记录时间、证据、影响、风险和后续操作。
- 故障现场会保存系统版本、准确时间、开机时长、磁盘、供电与进程快照，并写入本机健康时间线供后续 AI 复盘。
- AI 工具注册表和本地快速路由同步支持四项新能力，聊天中直接说“检查电池健康”“保存故障现场”等即可本机执行，无需 Hermes。
- 右侧新增持续任务轨道；后台任务不会阻塞文字或语音对话，最近状态在任何页面都可见。
- 修复纯文字发送仍会触碰 `AVAudioEngine.inputNode`、从而初始化麦克风和 CoreAudio 的遗漏；增加回归探针，空闲文字交互不会再访问音频输入。
- 电脑管家完整冒烟测试扩展为 13/13 项，并继续验证真实进程采样和可撤回文件整理执行器。

## v0.8.0 — 2026-07-19

- 文字、语音和系统通知改为严格分流：打字不会启动悬浮窗、麦克风或语音授权；授权卡出现时输入普通文字也不会绕过或替换待确认操作。
- Fn / 地球键改为明确长按约 0.3 秒后才启动收音，轻触不再误触语音；松开后仍会提交并恢复收音前音量。
- MiMo 混合识别不再依赖 Apple 语音识别授权：仅使用已授权麦克风录音，实时反馈真实声音活动，提交后由 MiMo 返回最终文字；Apple 权限已存在时继续提供逐句临时字幕。
- MiMo 密钥改由系统自带的稳定钥匙串代理读取，仅驻留进程内存；临时签名升级不再反复弹出开机密码窗口。
- 全套界面统一为深海 Liquid Glass 视觉：主界面、电脑管家、任务中心、设置、语音条、授权条和执行条使用一致层级、颜色、间距与按压反馈。
- 聊天页合并重复系统检测卡，健康时间线按类型展示最新事件；完整记录仍保留在本机并可供 AI 检索，不再让监控信息淹没正常对话。
- 电脑管家主界面的只读检测现在真正支持并行，并可从任务中心分别查看、计时和取消；清理、移动等修改操作继续安全排队。
- 主界面新增独立任务中心：只读检测可并行，文件与设置修改安全排队，跨应用任务不再互相锁死；每个后台任务显示状态、耗时、结果并可单独取消。
- 新增本机健康时间线，保存最多 200 条真实检测事件，明确区分正常、关注和风险；记录文件权限为仅当前用户可读。
- 新增低频自主维护：默认每 12 小时执行一次零模型额度的只读体检，低电量模式、语音通话或忙碌状态下自动跳过，只有异常才提醒。
- 智能整理新增可撤回事务：记录本次实际移动的源位置与目标位置，可一键恢复且不会覆盖同名文件。
- 本机新增打开常用应用能力，简单启动不再经过模型或 Hermes。
- 多轮会议等复杂任务会持续合并补充信息；“现在、立即、两个小时”等自然说法可以正确识别，避免每一轮都像新任务。
- 模型等待窗口由 24 秒提高到 42 秒，并对损坏 JSON 自动执行格式修复，协议内容不再原样泄漏到聊天窗口。
- 人格输出去除重复固定套话和居高临下的表达；保留嘴硬心软的风格，但技术、错误和风险信息优先清楚、尊重地说明。
- 支持从自然表达中记住“请用新手能懂的话解释”；电脑管家的发热进程结果会显示应用名称、影响和处理方式，不再直接堆完整进程路径。
- 同一发热进程提醒冷却时间延长至 4 小时；持续判定采用 45 秒低频采样，减少资源占用和重复打扰。
- 修正健康检测的状态颜色、扫描抽样范围说明、滚动截断、执行按钮命名、监控时长计算和重复历史卡片。
- 记忆设置与真实实现统一为最近 2–8 条即时上下文，并明确说明记忆存于本机、选中的相关内容会发送给当前云端模型。
- 开发自检不再读取钥匙串，避免构建签名变化时反复弹出系统密钥授权框。

## v0.7.4 — 2026-07-18

- 修复连续对话第二轮悬浮窗出现后，麦克风有音频但 Apple 识别任务不返回文字，最终被误判成静音的问题。
- 每轮语音使用全新的 Apple 识别会话，避免继承上一轮已取消的 Speech 后台连接。
- 连续对话优先保留下一轮正式识别通道；需要打断朗读时仍可按 Fn 立即插话。
- 混合识别增加真实声音兜底：检测到持续人声后，即使 Apple 没有输出临时文字，也会把本轮录音交给 MiMo 识别，不再直接丢弃。
- 悬浮窗会先显示“听到声音，正在识别…”，明确反馈麦克风已经收到说话声。
- 连续对话支持语音挂断；说“结束对话、关闭对话、停止对话、退出语音、挂断”等明确口令会立即停止播报和收音并关闭悬浮窗。
- 授权后的 Hermes 与本机操作进入独立后台任务槽；执行卡短暂展示后恢复语音通话，不再要求用户一直盯着进度卡。
- 后台任务运行期间可以继续语音提问；只有明确说“暂停当前任务、取消这个任务、修改当前任务”等口令才会影响后台任务。
- 悬浮语音窗显示后台任务运行、完成或失败状态，任务结果同步进聊天记录但不会抢占正在进行的语音交互。
- 每条用户命令、助手回复、任务状态与错误都会以原始 `createdAt` 保存，并向 AI 渲染绝对日期、精确时间、时区以及“今天/昨天/几天前”的关系。
- 当前工作任务同时保存创建时间和最后更新时间，旧任务不会因为后续更新而被误认为刚刚创建。
- 历史检索新增时间范围理解；“昨天那个任务、前天的操作、今天的命令”等说法会优先检索对应日期的真实记录。
- 后台任务记录开始时间和最后一次真实进度时间；连续 5 分钟没有进度会从“执行中”变为“可能卡住”，并记录带时间的异常提醒。
- AI 上下文始终包含后台任务已运行多久、最后进度时间和卡住判断；询问“任务多久了、还正常吗”会直接读取真实运行状态，不再机械回答正常。
- 新增 Mac 专家知识检索层，覆盖性能发热、存储与 APFS、文件整理、隐私权限、CoreAudio、网络外设、launchd、备份更新等专题，只向模型提供当前问题相关规则。
- 新增本机经验学习库，只记录真实工具与 Hermes 的成功/失败结果，并绑定 macOS 版本、芯片架构和发生时间；相同问题会优先参考这台 Mac 上已经验证过的方法。
- 系统升级后旧经验自动标记为“必须重新验证”，不会把旧系统方案机械照搬；高风险操作的授权边界不因学习而放宽。
- Mac 专家知识扩展为二十六个独立 Skill，新增电池、电源、内存压力、应用崩溃、内核恐慌、磁盘健康、Time Machine、iCloud、Spotlight、安全公证、显示器、蓝牙、USB/雷雳、睡眠唤醒、系统日志、应用生命周期、浏览器、快捷指令和打印扫描等专题。
- Skill 索引只向模型提供名称目录，每次最多按需加载一个相关正文；普通聊天不会加载复杂知识，在扩大专业覆盖面的同时控制上下文与等待。
- 新增混合决策权衡：状态读取、扫描和简单控制走本机快速通道；原因、影响、风险与方案分析由 MiMo 结合本机证据回答；只有跨应用复杂执行才使用 Hermes。
- 每次电脑管家检测都会生成结构化责任卡，记录检测时间、来源、证据、影响、风险等级、处理归属和下一步；后续追问无需重新扫描或翻聊天记录。
- 发热监控的持续高负载提醒会同步为真实电脑管家报告，直接说明影响和处理边界，并进入助手共享上下文供 MiMo 继续分析。
- 新增可切换内置人格“绾宁 · 古来客”：误入Mac的古代少女，毒舌细心、嘴硬心软，最信任用户但会拦截危险操作；完整身世、成长与关系规则同时作用于文字和语音。
- 人格只改变表达、称呼和语音风格，不改变Mac工具、诊断事实、授权机制或安全边界；技术和风险场景始终优先准确清楚。
- 修复选择内置人格后仍出现通用助理口吻的问题：当前人格升级为模型输出契约，并在统一回复出口为本机快速回答补充人格表达；文字与语音使用同一人格状态，切回其他人格后不会残留。
- 人格系统改为渐进加载：日常请求只携带精简说话风格和一行人物档案索引；只有询问绾宁的身份、身世、穿越、关系或剧情时，才从独立人物Skill读取完整背景，不再让普通聊天重复携带整篇故事。

- Fixed follow-up turns being discarded when Apple Speech produced no partial transcript.
- Added fresh per-turn recognizers and a MiMo recording fallback for detected voice activity.
...
- 恢复只在当前音量低于原值时执行；如果用户在收音期间主动把音量调高，浮屿不会再把它调低。
- 连续对话进入下一轮前会等待上一轮音量恢复完成，避免把临时压低后的音量误记成新的基准。
- 连续对话第二轮会在 CoreAudio 完成上一轮释放后重新建立收音；启动后 900 毫秒仍没有任何音频缓冲时自动重建一次麦克风管线。
- 重建后仍无音频会明确提示麦克风没有收到数据，不再假装收音并在静音超时后无声关闭。
- 新增连续麦克风真机测试；本次发布验证第一轮和第二轮均收到真实 CoreAudio 音频缓冲，测试结束后音量恢复到原值。

- Changed Fn audio ducking to restore the pre-listening output level only after capture ends.
...
- 每层直接显示当前状态，包括即时轮数、是否跨重启、归档调用状态和永久习惯数量。
- 更新所有旧版记忆文案，明确较早内容会进入本机归档并按相关性检索。
- 修正清除记忆说明：清除对话、当前任务和会话归档，但不会误删永久习惯。

- Updated Memory settings to reflect the new four-layer architecture and clarify deletion scope.
